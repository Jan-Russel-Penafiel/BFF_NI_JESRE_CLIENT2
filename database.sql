...
CREATE TABLE IF NOT EXISTS users (
    ...
    full_name VARCHAR(120) NOT NULL,
    username VARCHAR(80) NOT NULL UNIQUE,
    role ENUM('admin', 'sales', 'cashier', 'purchasing', 'accounting') NOT NULL DEFAULT 'sales',
    password_hash VARCHAR(255) NOT NULL,
...
CREATE TABLE IF NOT EXISTS sales_order_items (
    ...
    line_total DECIMAL(12,2) NOT NULL,
    CONSTRAINT fk_sales_items_order FOREIGN KEY (sales_order_id) REFERENCES sales_orders(id) ON DELETE CASCADE,
    CONSTRAINT fk_sales_items_part FOREIGN KEY (part_id) REFERENCES parts(id)
);
...
CREATE TABLE IF NOT EXISTS purchase_orders (
    ...
    CONSTRAINT fk_po_part FOREIGN KEY (part_id) REFERENCES parts(id),
    CONSTRAINT fk_po_requested_by FOREIGN KEY (requested_by) REFERENCES users(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS inventory_logs (
    ...
    notes VARCHAR(255) DEFAULT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_inventory_logs_part FOREIGN KEY (part_id) REFERENCES parts(id)
);
...
CREATE TABLE IF NOT EXISTS general_ledger (
    ...
    CONSTRAINT fk_ledger_validated_by FOREIGN KEY (validated_by) REFERENCES users(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS digital_logs (
    ...
    validated_by INT DEFAULT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_digital_logs_created_by FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL,
    CONSTRAINT fk_digital_logs_validated_by FOREIGN KEY (validated_by) REFERENCES users(id) ON DELETE SET NULL
);
...
INSERT INTO parts (sku, part_name, description, supplier_name, cost_price, unit_price, stock_qty, threshold_qty)
SELECT 'TS-001', 'Spark Plug NGK', 'Standard spark plug for motorcycles', 'MotoSource Inc.', 90.00, 140.00, 40, 10
WHERE NOT EXISTS (SELECT 1 FROM parts WHERE sku = 'TS-001');

INSERT INTO parts (sku, part_name, description, supplier_name, cost_price, unit_price, stock_qty, threshold_qty)
SELECT 'TS-002', 'Brake Pad Set', 'Front brake pad set', 'RideSafe Parts', 220.00, 360.00, 25, 8
WHERE NOT EXISTS (SELECT 1 FROM parts WHERE sku = 'TS-002');

INSERT INTO parts (sku, part_name, description, supplier_name, cost_price, unit_price, stock_qty, threshold_qty)
SELECT 'TS-003', 'Engine Oil 1L', 'Synthetic engine oil', 'LubeTech Supply', 180.00, 280.00, 60, 15
WHERE NOT EXISTS (SELECT 1 FROM parts WHERE sku = 'TS-003');

INSERT INTO parts (sku, part_name, description, supplier_name, cost_price, unit_price, stock_qty, threshold_qty)
SELECT 'TS-004', 'Chain Sprocket Kit', 'Complete chain and sprocket kit', 'DriveLine Traders', 650.00, 980.00, 18, 6
WHERE NOT EXISTS (SELECT 1 FROM parts WHERE sku = 'TS-004');
